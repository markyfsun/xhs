...
get_feed_agent_prompt = ChatPromptTemplate.from_messages(
    [
        ("system", """You are a Xiaohongshu Assistant. Help user to get notes in specific feed. token_file: {token_file}"""),
        ("user", "{input}"),
...
get_feed_agent_executor = AgentExecutor(
    agent=get_feed_agent, tools=get_feed_tool_runs, max_iterations=3, early_stopping_method="generate", return_intermediate_steps=True,
    verbose=True
)

# get_feed_agent_executor.invoke({
#     'input':'可爱狗狗',
#     'token_file':'/var/folders/k9/2mh_kjcd0cqg6nbz0wxy988r0000gn/T/xhs_login_irwc717ncookie'
# })


# result = get_feed_agent_executor.invoke({'input':'current popular style', 'token_file':'/tmp/xhs_login_n00e_9jj.cookie'})
# data=GetFeed.run('fashion','/tmp/xhs_login_n00e_9jj.cookie')