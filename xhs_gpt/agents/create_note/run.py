...
create_image_tool_runs = [Tool.from_function(func=i.run, name=i.tool['name'], description=i.tool['description']) for i in (
    CreateImage,
)]
...
create_note_agent_prompt = ChatPromptTemplate.from_messages(
    [
        ("system", """You are a Xiaohongshu Assistant. Help user to refine their topic, then create a note. token_file: {token_file}"""),
        ("user", "{input}"),
        MessagesPlaceholder(variable_name="agent_scratchpad"),
    ]
)
...
create_note_agent = (
        {
            "input": lambda x: x["input"],
            "token_file": lambda x:x["token_file"],
            "agent_scratchpad": lambda x: format_to_openai_functions(
                x["intermediate_steps"]
            ),
        }
...
create_note_agent_executor = AgentExecutor(
    agent=create_note_agent, tools=create_note_tool_runs, max_iterations=3, early_stopping_method="generate", return_intermediate_steps=True,
    verbose=True
)

# create_note_agent_executor.invoke({
#     'input':'可爱狗狗',
#     'token_file':'/var/folders/k9/2mh_kjcd0cqg6nbz0wxy988r0000gn/T/xhs_login_irwc717ncookie'
# })
